...
def main(args):
    ...
    with open(args.vocab_path, 'rb') as f:
        vocab = pickle.load(f)
    train_story_keys = pickle.load( open( "train_story_keys", "rb" ) )
    val_story_keys = pickle.load( open( "validation_story_keys", "rb" ) )
    
    train_src_descs = pickle.load( open( "all_training_description", "rb" ) )
    val_src_descs = pickle.load( open( "all_validation_description", "rb" ) )
    # Build data loader
    train_data_loader = get_loader(args.train_image_dir, args.train_sis_path, train_story_keys, train_src_descs, vocab, train_transform, args.batch_size, shuffle=True, num_workers=args.num_workers)
    val_data_loader = get_loader(args.val_image_dir, args.val_sis_path, val_story_keys, val_src_descs, vocab, val_transform, args.batch_size, shuffle=False, num_workers=args.num_workers)

    encoder = EncoderStory(args.img_feature_size, args.hidden_size, args.num_layers)
    #desc_encoder = EncoderDescription(args.embed_size, args.hidden_size, vocab)
    ...
    for epoch in range(args.num_epochs):
        ...
        decoder.train()
        avg_loss = 0.0
        for bi, (image_stories, targets_set, lengths_set, src_desc_set, desc_lengths_set, photo_squence_set, album_ids_set) in enumerate(train_data_loader):
            decoder.zero_grad()
            #desc_encoder.zero_grad()
            encoder.zero_grad()
            loss = 0
            images = to_var(torch.stack(image_stories))

            features, i_hidden = encoder(images)
            
            for si, data in enumerate(zip(features, targets_set, lengths_set, src_desc_set, desc_lengths_set)):
                feature = data[0]
                captions = to_var(data[1])
                lengths = data[2]
                src_desc = to_var(data[3])
                desc_lengths = data[4]
                
                #desc_output, (d_hn, d_cn) = desc_encoder((d_hn, d_cn), src_desc, desc_lengths)
        ...
        for bi, (image_stories, targets_set, lengths_set, src_desc_set, desc_lengths_set, photo_sequence_set, album_ids_set) in enumerate(val_data_loader):
            loss = 0
            images = to_var(torch.stack(image_stories))
