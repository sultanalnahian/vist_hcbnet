...
class EncoderCNN(nn.Module):
    def __init__(self, target_size):
        ...
        num_features = self.resnet.classifier[6].in_features
        #modules = list(resnet.children())[:-1]
        modules = list(self.resnet.classifier.children())[:-1]
        self.resnet.classifier = nn.Sequential(*modules)
        for param in self.resnet.features.parameters():
            param.requires_grad = False

        #self.linear = nn.Linear(resnet.fc.in_features, target_size)
        self.linear = nn.Linear(num_features, target_size)
