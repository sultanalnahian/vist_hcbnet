...
class DecoderRNN(nn.Module):
    def __init__(self, embed_size, hidden_size, n_layers, vocab):
        ...
        self.vocab = vocab
        self.desc_emb_size = 256
        vocab_size = len(vocab)

        #word_embedding_weight = pickle.load( open( "glove_word_embedding", "rb" ), encoding='ISO-8859-1' )
        #tensor_weights = torch.FloatTensor(word_embedding_weight)
        self.embed = nn.Embedding(vocab_size, embed_size)
        #self.embed.weight = nn.Parameter(tensor_weights)
        
        #self.embed = nn.Embedding(vocab_size, embed_size)
        self.dropout1 = nn.Dropout(p=0.1)
        #self.lstm = nn.LSTM(embed_size + hidden_size + self.desc_emb_size, hidden_size, n_layers, batch_first=True, dropout=0.5)
        ...
        self.desc_lstm = nn.LSTM(embed_size, hidden_size, n_layers, batch_first=True, dropout=0.5)
        ...
        #self.desc_emb_linear = nn.Linear(hidden_size*n_layers*2, self.desc_emb_size)
        self.brobs = []
    ...
    def init_weights(self):
        ...
        #self.desc_emb_linear.weight.data.normal_(0.0, 0.02)
        #self.desc_emb_linear.bias.data.fill_(0) 
    ...
    def forward(self, features, captions, lengths, desc, desc_lengths):
        ...
        for i, length in enumerate(lengths):
            ...
            #desc_embed = torch.cat((d_hn, d_cn), 0)
            #desc_embed = desc_embed.view(-1)
            #desc_embed = self.desc_emb_linear(desc_embed)
            
            lstm_input = embeddings[i][0:length - 1]
            desc_expand_size = lstm_input.size()[0]
            
            #desc_embed_resize = desc_embed.expand(desc_expand_size,-1)
            #lstm_input = torch.cat((desc_embed_resize,lstm_input),1)
    ...
    def inference(self, features, desc, desc_lengths):
        ...
        for feature in features:
            ...
            #desc_embed = desc_embed.view(-1)
            #desc_embed = self.desc_emb_linear(desc_embed)
            #desc_embed = desc_embed.unsqueeze(0).unsqueeze(0)

            #lstm_input = torch.cat((desc_embed,lstm_input),2)
